...
- **路径与目录智能解析规则**：
  - **点号 `.` 就地展开**：例如 `leoms create <tpl> .`，直接在当前目录释放文件（项目名自动取当前目录名）。
  - **工作区根目录下**：传入纯项目名时（如 `my-app`），自动规范收纳到 `apps/<name>` 目录下（即 `apps/my-app`）。
  - **任意子目录下**：默认直接在当前执行命令的目录下新建文件夹 `./<name>`。
  - **相对/绝对路径**：支持指定相对路径（如 `./packages/ui-lib`）或绝对路径（如 `/opt/projects/my-service`），系统自动将最后一个斜杠后的名称（`basename`）作为最终项目名。
- **模板匹配与冒号协议**：
